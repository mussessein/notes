...
1. 一对一关系：一个RDD分区对应一个父RDD分区（map，filter）
2. 多对一关系：一个RDD分区对应多个父RDD分区（partitioned）

### 宽依赖

1. 一对多关系：一个父RDD分区对应多个子RDD分区（groupByKey）

![](image/stage1.png)

- 窄依赖之间可以并行执行；
- 宽依赖必须所有父RDD执行完毕，才能执行子RDD；
- 数据恢复问题：窄依赖只需要一个RDD来恢复数据，宽依赖需要多个RDD一起来恢复数据，所以，宽依赖需要适当设置**数据检查点**；（checkpoint）
...
- 如果cache失效，仍能通过血缘拿到数据
- 如果cache有效，就可以不计算之前的数据，直接使用，节省资源；
...
一种容灾措施；在SparkStreaming中高可用，有具体使用；
...
- Hash Shuffle：2.x之前版本
- Sort Shuffle：2.x之后

### Spark Shuffle过程

官方shuffle参数：http://spark.apache.org/docs/latest/configuration.html#shuffle-behavior
